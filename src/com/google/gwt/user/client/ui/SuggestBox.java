 ...
 * the cell }</li>
 * 
 * <li>.gwt-SuggestBoxPopup .suggestPopupMiddleCenter { the middle center cell }
 * </li>
 * 
 ...
 * the cell }</li>
 * 
 * <li>.gwt-SuggestBoxPopup .suggestPopupBottomCenter { the bottom center cell }
 * </li>
 * 
...
public final class SuggestBox extends Composite implements HasText, HasFocus, HasAnimation, SourcesClickEvents, SourcesFocusEvents, SourcesChangeEvents, SourcesKeyboardEvents, FiresSuggestionEvents {
	 ...
	 * appear on top of any other widgets on the page, and the PopupPanel
	 * provides this behavior.
	 *
	 * A non-static member class is used because the popup uses the SuggestBox's
	 * SuggestionMenu as its widget, and the position of the SuggestBox's
	 * TextBox is needed in order to correctly position the popup.
	 */
	public class SuggestionPopup extends DecoratedPopupPanel {
		...
		public SuggestionPopup() {
			...
			scrollPanel.setPixelSize(620, 360);
		 ...
		 * SuggestBox's text box, with its left edge aligned with the left edge
		 * of the text box. Depending on the width and height of the popup and
		 * the distance from the text box to the bottom and right edges of the
		 * window, the popup may be displayed directly above the text box,
		 * and/or its right edge may be aligned with the right edge of the text
		 * box.
		 */
			...
			setPopupPositionAndShow(new PositionCallback() {
				...
				public void setPosition(int offsetWidth, int offsetHeight) {
					...
					if (LocaleInfo.getCurrentLocale().isRTL()) { // RTL case
						...
						// If the suggestion popup is not as wide as the text
						// box, always align
						// to the left edge of the text box. Otherwise, figure
						// out whether to
						...
						if (offsetWidthDiff > 0) {
							...
							// Distance from the left edge of the text box to
							...
							// If there is not enough space for the overflow of
							// the popup's width to
							// the right of hte text box, and there IS enough
							// space for the overflow
							// to the left of the text box, then right-align the
							// popup. However, if
							// there is not enough space on either side, then
							// stick with left-alignment.
	...
	private void showSuggestions(Collection<? extends Suggestion> suggestions) {
		...
		if (suggestions.size() > 0) {
			...
			for (Suggestion curSuggestion : suggestions) {
				...
				final SuggestionMenuItem menuItem = new SuggestionMenuItem(curSuggestion, oracle.isDisplayStringHTML());menuItem.setCommand(new Command() {
