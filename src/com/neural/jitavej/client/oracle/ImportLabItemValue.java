...
*/
public class ImportLabItemValue {
/*
	public static void main(String[] args) {
		...
		//	Connection connection = DriverManager.getConnection("jdbc:oracle:oci8:@hims", "sys", "sysistrue");
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM hims.L_SRVITEM");
			URL url = new URL("http://localhost:8080/jitavej/lab/importlabitemvalue");
	    	
			while (rs.next()) {
			   try {

			        String data = URLEncoder.encode("value_id", "UTF-8") + "=" + URLEncoder.encode(ThaiEncodingUtils.ASCII2Unicode(rs.getString(1).trim()), "UTF-8");
			        data += "&" + URLEncoder.encode("item_id", "UTF-8") + "=" + URLEncoder.encode(ThaiEncodingUtils.ASCII2Unicode(rs.getString(2).trim()), "UTF-8");
			        String order = "0";
					try {
						order = rs.getString(3).trim();
					} catch (Exception e) {
					}
