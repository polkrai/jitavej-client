...
public class MedErrorDialog extends FormDialog {
	public TextBox text;
	
	
	public MedErrorDialog() {
		setHTML("<b>"+Jitavej.CONSTANTS.patient_mederror()+"</b>:");
		...
		try{
			text.setText(Jitavej.patient.get("mederror").isString().stringValue());			
		}catch(Exception ex){}
		
		Button submit1 = new Button(Jitavej.CONSTANTS.appointment_save(), new ClickListener() {
			public void onClick(Widget sender) {
				try {	
					RequestBuilder rb = new RequestBuilder(RequestBuilder.POST, Jitavej.server + "/patient/setmederror");
					rb.setTimeoutMillis(30000);	
					StringBuffer params = new StringBuffer();
					params.append("patient_id=" + URL.encodeComponent(Jitavej.patient.get("id").isNumber().toString()));
					params.append("&");
					params.append("mederror=" + URL.encodeComponent(text.getText().trim()));
