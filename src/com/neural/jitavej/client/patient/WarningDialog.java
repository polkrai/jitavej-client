...
public class WarningDialog extends FormDialog {
	
	public TextBox text;
	
	public WarningDialog() {
		
		setHTML("<b>"+Jitavej.CONSTANTS.patient_warning()+"</b>:");
		
		DecoratedTabPanel tabPanel = new DecoratedTabPanel();

		text = new TextBox();
		text.setPixelSize(400, 18);
		
		try{
			
			text.setText(Jitavej.patient.get("warning").isString().stringValue());	
		}
		catch(Exception ex){
		...
		Button submit1 = new Button(Jitavej.CONSTANTS.appointment_save(), new ClickListener() {
			...
			public void onClick(Widget sender) {
				...
				try {	
					RequestBuilder rb = new RequestBuilder(RequestBuilder.POST, Jitavej.server + "/patient/setwarning");
					rb.setTimeoutMillis(30000);	
					StringBuffer params = new StringBuffer();
					params.append("patient_id=" + URL.encodeComponent(Jitavej.patient.get("id").isNumber().toString()));
					params.append("&");
					params.append("warning=" + URL.encodeComponent(text.getText().trim()));
					...
					rb.sendRequest(params.toString(), new RequestCallback() {
						...
						public void onResponseReceived(Request request, Response response) {
							if (response.getStatusCode() == 200) {
								Jitavej.patient.put("warning", new JSONString(text.getText().trim()));
